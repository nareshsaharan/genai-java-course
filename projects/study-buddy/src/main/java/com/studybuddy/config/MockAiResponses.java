...
/**
 * Canned text used by {@link DynamicAnthropicChatModel} when no Anthropic key
 * is configured for the current session (Mock Mode). LangChain4j's
 ...
 * proxies parse it the same way they'd parse a real Claude response.
 */
final class MockAiResponses {
    ...
    private static String tutorAnswer() {
        return "This is a demo answer from Study Buddy's Mock Mode. No Claude API key is "
                + "configured for this session, so no real AI call was made and this text isn't "
                + "grounded in your uploaded course material. Add your Anthropic API key in the "
                + "Settings tab to get real, document-grounded answers.";
    }

    private static String flashcards() {
        return "Mock Mode is active (no Claude API key configured), so these are example "
                ...
                + "\"answer\":\"Add your Anthropic API key in the Settings tab.\"},"
    ...
    private static String quiz() {
        return "Mock Mode is active (no Claude API key configured), so this is an example quiz, "
                ...
                + "\"options\":[\"Wait 24 hours\",\"Add your Anthropic API key in Settings\",\"Reinstall the app\",\"Nothing needed\"],"
